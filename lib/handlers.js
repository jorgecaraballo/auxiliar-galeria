...
handlers.apiOpen = function(data, callback) {
	...
	fs.stat(absolutePath, function(err) {
		...
		// execFile pasa los argumentos directamente al proceso sin pasar por el shell,
		// así las rutas con espacios o caracteres especiales funcionan sin escapar nada.
		childProcess.execFile('pcmanfm', ['--select', absolutePath], function(err) {
			if (err) {
