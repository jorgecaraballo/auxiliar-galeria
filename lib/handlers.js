...
handlers.apiOpen = function(data, callback) {
	...
	fs.stat(absolutePath, function(err) {
		...
		// pcmanfm --select abre el gestor en la carpeta con el archivo ya seleccionado
		var cmd = 'pcmanfm --select ' + JSON.stringify(absolutePath);
		debug('Abriendo PCManFM:', cmd);
