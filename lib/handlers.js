...
handlers.apiOpen = function(data, callback) {
	...
	fs.stat(absolutePath, function(err) {
		...
		// Abre PCManFM en la carpeta que contiene el archivo.
		// pcmanfm --select no está disponible en todas las versiones; abrir
		// la carpeta directamente es más compatible y el archivo queda visible.
		var dir = path.dirname(absolutePath);
		var env = Object.assign({}, process.env, { 'DISPLAY': process.env.DISPLAY || ':0' });
		childProcess.execFile('pcmanfm', [dir], { 'env': env }, function(err) {
			if (err) {
				debug('Error al abrir PCManFM:', err.message);
				return callback(500, { 'error': 'No se pudo abrir PCManFM: ' + err.message });
			}
			callback(200, { 'ok': true, 'path': absolutePath });
		});
	});
};
