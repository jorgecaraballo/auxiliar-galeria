...
handlers.apiOpen = function(data, callback) {
	...
	fs.stat(absolutePath, function(err) {
		...
		childProcess.execFile('dbus-send', [
			...
			if (err) {
				// Fallback: abrir la carpeta sin seleccionar
				debug('D-Bus ShowItems falló, abriendo carpeta:', err.message);
				var dir = path.dirname(absolutePath);
				childProcess.execFile('pcmanfm', [dir], { 'env': env }, function(err2) {
					if (err2) {
						return callback(500, { 'error': 'No se pudo abrir PCManFM: ' + err2.message });
					}
