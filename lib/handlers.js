...
handlers.apiOpen = function(data, callback) {
	...
	fs.stat(absolutePath, function(err) {
		...
		// Lanza pcmanfm-qt en background para que registre el servicio D-Bus
		// org.freedesktop.FileManager1. Sin una instancia corriendo, dbus-send
		// no tiene a quién enviarle el mensaje y falla silenciosamente.
		childProcess.execFile('pcmanfm-qt', [], { 'env': env, 'detached': true, 'stdio': 'ignore' }, function() {});

		// Espera 800ms a que pcmanfm-qt arranque y se registre en D-Bus,
		// luego pide que muestre y seleccione el archivo.
		setTimeout(function() {
