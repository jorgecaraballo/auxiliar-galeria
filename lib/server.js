...
server.restante = function(chosenHandler, data, buffer, req, res) {
	chosenHandler(data, function(statusCode, payload, contentType, extraHeaders) {
		try {
			contentType = typeof(contentType) == 'string' ? contentType : 'json';
			statusCode = typeof(statusCode) == 'number' ? statusCode : 200;
			extraHeaders = typeof(extraHeaders) == 'object' && extraHeaders !== null ? extraHeaders : {};
			...
			// Video con Range request (HTTP 206 Partial Content)
			if (contentType == 'videoRange' || contentType == 'videoFull') {
				Object.keys(extraHeaders).forEach(function(h) {
					res.setHeader(h, extraHeaders[h]);
				});
				payloadString = typeof(payload) !== 'undefined' ? payload : '';
			}
...
server.router = {
	...
	'api/videos': handlers.apiVideos,
	'api/rescan-videos': handlers.apiRescanVideos,
	'video': handlers.video,
	'api/open': handlers.apiOpen,
};
...
server.init = function() {
	server.httpServer.listen(config.httpPort, function() {
		...
		watcher.init(io);
		// Arranca el escaneo + polling de la carpeta de videos
		videoWatcher.init(io);
	});
};
